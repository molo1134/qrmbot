...
# Contest Online ScoreBoard lookup: !scores <callsign>
...
# Returns list of {id name} pairs for all "On air" contests
proc cosb_get_active {html} {
	set active {}
	set re {<option[^>]*value=['"](\d+)['"][^>]*>On air:\s*([^<]+)</option>}
	set pos 0
	while {[regexp -nocase -start $pos -indices $re $html range id_idx name_idx]} {
		set id   [string trim [string range $html {*}$id_idx]]
		set name [string trim [string range $html {*}$name_idx]]
		lappend active [list $id $name]
		set pos [expr {[lindex $range 1] + 1}]
	}
	return $active
}

# Returns {rank score qsos} for call in scoreboard html, or {} if not found
proc cosb_find_call {html call} {
	set lhtml [string tolower $html]
	set lcall [string tolower $call]

	set link_pos [string first "call=${lcall}" $lhtml]
	if {$link_pos < 0} { return {} }

	set tr_pos [string last "<tr" [string range $html 0 $link_pos]]
	if {$tr_pos < 0} { return {} }

	set tr_end [string first "</tr>" $html $link_pos]
	if {$tr_end < 0} { return {} }
	set row [string range $html $tr_pos [expr {$tr_end + 4}]]

	if {![regexp -nocase {<td[^>]*>(\d+)} $row -> rank]} { return {} }

	set call_in_row [string first "call=${lcall}" [string tolower $row]]
	set after_td_pos [expr {[string first "</td>" $row $call_in_row] + 5}]
	set after_call [string range $row $after_td_pos end]

	if {![regexp -nocase {<td[^>]*>([0-9,]+)[^<]*</td>} $after_call -> score]} { return {} }
	if {![regexp -nocase {<td[^>]*>[^<]*<strong>([0-9,]+)} $after_call -> qsos]} { return {} }

	set score [string map {"," ""} $score]
	set qsos  [string map {"," ""} $qsos]
	return [list $rank $score $qsos]
}

proc scores_pub { nick host hand chan text } {
	global cosb_base
	set call [string toupper [sanitize_string [string trim "${text}"]]]
	if {$call eq ""} {
		putchan $chan "usage: !scores <callsign>"
		return
	}
	putlog "scores pub: $nick $host $hand $chan $call"

	set html [cosb_fetch $cosb_base]
	if {$html eq ""} {
		putchan $chan "Error: couldn't reach contestonlinescore.com"
		return
	}

	set active [cosb_get_active $html]
	if {[llength $active] == 0} {
		putchan $chan "No contests currently on air at COSB."
		return
	}

	set found 0
	foreach contest $active {
		lassign $contest cid cname
		set chtml [cosb_fetch "${cosb_base}?contest_id=${cid}"]
		if {$chtml eq ""} continue
		set result [cosb_find_call $chtml $call]
		if {[llength $result]} {
			lassign $result rank score qsos
			putchan $chan "COSB \[${cname}\]: ${call} #${rank} | Score: ${score} | QSOs: ${qsos}"
			set found 1
		}
	}

	if {!$found} {
		set names [lmap c $active {lindex $c 1}]
		putchan $chan "${call} not found in active COSB contest(s): [join $names {, }]"
	}
}

proc scores_msg {nick uhand handle input} {
	global cosb_base
	set call [string toupper [sanitize_string [string trim "${input}"]]]
	if {$call eq ""} {
		putmsg $nick "usage: !scores <callsign>"
		return
	}
	...
	set html [cosb_fetch $cosb_base]
	if {$html eq ""} {
		putmsg $nick "Error: couldn't reach contestonlinescore.com"
		return
	}

	set active [cosb_get_active $html]
	if {[llength $active] == 0} {
		putmsg $nick "No contests currently on air at COSB."
		return
	}
	...
	foreach contest $active {
		...
		if {[llength $result]} {
			lassign $result rank score qsos
			putmsg $nick "COSB \[${cname}\]: ${call} #${rank} | Score: ${score} | QSOs: ${qsos}"
			set found 1
		}
	}
	...
	if {!$found} {
		set names [lmap c $active {lindex $c 1}]
		putmsg $nick "${call} not found in active COSB contest(s): [join $names {, }]"
	}
}
